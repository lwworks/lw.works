...
export const Card: FC<{children: ReactNode; className: string; borderAnimation?: boolean | false}> = ({children, className, borderAnimation}) => {
  return (
    <div className="relative overflow-hidden w-full h-full p-px rounded-[2.5rem] bg-gradient-to-b from-slate-200 to-slate-100 dark:from-[#393C45] dark:to-[#1A1D24]">
      ...
      <div className={`relative w-full rounded-[39px] bg-[#F9FAFB] dark:bg-[#101318] h-full ${className}`}>{children}</div>
    </div>
  )
}
